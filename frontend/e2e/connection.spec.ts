...
test("страница показывает полученные от backend данные", async ({ page, request }) => {
  ...
  await expect(page.getByTestId("intervention-count")).toHaveText(
    String(catalog.city.interventions.length),
  );
  await expect(page.getByTestId("baseline-score")).toBeVisible();
});

test("ошибка каталога видна пользователю и повторный запрос восстанавливает страницу", async ({
  page,
}) => {
  ...
  await page.goto("/");
  const connectionError = page
    .getByRole("region", { name: "Подключение к серверу" })
    .getByRole("alert");
